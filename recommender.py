...
COFFEE_MODEL = joblib.load(MODEL_PATH) if os.path.exists(MODEL_PATH) else None
...
def recommend_optimized(
    ...
    user_info = users_df.loc[user_id]
    
    available_recipes = filter_by_equipment(user_info['owned_equipment'], recipes_df).copy()
    
    if available_recipes.empty:
        return []

    is_warm = user_id in train_df['user_id'].values 
    
    if not is_warm or COFFEE_MODEL is None:
    ...
    for t in tastes:
        pref_val = user_info[f'taste_pref_{t}']
        X_predict[f'taste_pref_{t}'] = pref_val
        X_predict[f'delta_{t}'] = (X_predict[f'taste_{t}'] - pref_val).abs()
    ...
    scores = COFFEE_MODEL.predict(X_predict[feature_cols])
    X_predict['score'] = scores
    
    res = X_predict.nlargest(n, 'score')
